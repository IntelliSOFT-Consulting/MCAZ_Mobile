  ...
  render() {
    ...
    return (
      <ScrollView style={ AppStyles.scrollContainer }>
        ...
        <SAEDrugsTableComponent model={ model } name="adr_list_of_drugs"/>
        <SelectOneField label="9. Was the patient taking any other drug at the time of onset of the AE?" model={ model } name="patient_other_drug" options={["Yes", "No"]}/>
        ...
        <SAEConcomitantTableComponent model={ model } name="adr_other_drugs"/>
        <Text>11. Has the Adverse Event been
          reported to:</Text>
        <SelectOneField label="(a) MCAZ" model={ model } name="report_to_mcaz" options={["Yes", "No"]}/>
        <DateTimeInput label="Date" model={ model } name="report_to_mcaz_date"/>
        <SelectOneField label="(b) MRCZ" model={ model } name="report_to_mcrz" options={["Yes", "No"]}/>
        <DateTimeInput label="Date" model={ model } name="report_to_mcrz_date"/>
        <SelectOneField label="(c) Sponsor" model={ model } name="report_to_sponsor" options={["Yes", "No"]}/>
        <DateTimeInput label="Date" model={ model } name="report_to_sponsor_date"/>
        <SelectOneField label="(d) IRB" model={ model } name="report_to_irb" options={["Yes", "No"]}/>
        <DateTimeInput label="Date" model={ model } name="report_to_irb_date"/>
        ...
        <TextInputField label="(d) Investigations-Laboratory and any other significant
          ...
          numberOfLines = {4}/>
        <LabsTableComponent model={ model } name="adr_lab_tests"/>
        <TextInputField label="(e) Results:" multiline = {true} model={ model } name="results"
          numberOfLines = {4}/>
