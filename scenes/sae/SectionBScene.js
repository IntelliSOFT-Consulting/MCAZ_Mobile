  ...
  render() {
    ...
    return (
      <KeyboardAwareScrollView style={ [ AppStyles.scrollContainer, AppStyles.sadrBackground ] } keyboardShouldPersistTaps={'handled'}>
        ...
        <SelectOneField label="2a. If SAE, is it" name="sae_type" model={ model } validate={ this.state.validate } options={ SAE_EVENT_TYPE } onChange={this.onChange} />
        <TextInputField label="If Other, specify" name="sae_description" model={ model } validate={ this.state.validate } tintColor="rgba(0, 0, 0, .60)" onChange={this.onChange} />
        <SelectOneField label="2b. Toxicity Grade:" name="toxicity_grade" model={ model } validate={ this.state.validate } options={ SAE_TOXICITY_GRADE } onChange={this.onChange} />
        <SelectOneField label="3a. Any previous Adverse Event’s report on this participant?:" name="previous_events" model={ model } validate={ this.state.validate } options={ BOOLEAN_OPTIONS } onChange={ this.onSelectChange }/>
        {
          ...
          (<TextInputField label="If yes, how many?" name="previous_events_number" keyboardType="numeric" model={ model } validate={ this.state.validate } tintColor="rgba(0, 0, 0, .60)" onChange={this.onChange} />)
        }
        <TextInputField label="3b. Total Number of SAEs to date for the whole study:" keyboardType="numeric" name="total_saes" model={ model } validate={ this.state.validate } tintColor="rgba(0, 0, 0, .60)" onChange={this.onChange} />
        ...
        <SelectOneField label="5. Research involves a:" name="research_involves" model={ model } validate={ this.state.validate } options={ RESEARCH_INVOLVES } onChange={ this.onSelectChange }/>
        {
          ...
          (<TextInputField label="If Other, specify" name="research_involves_specify" model={ model } validate={ this.state.validate } tintColor="rgba(0, 0, 0, .60)" onChange={this.onChange} />)
        }
        <TextInputField label="6. Name of Drug, Device or Procedure:" name="name_of_drug" model={ model } validate={ this.state.validate } tintColor="rgba(0, 0, 0, .60)" onChange={this.onChange} />
        <SelectOneField label="7. Is the drug/device investigational:" name="drug_investigational" model={ model } validate={ this.state.validate } options={ BOOLEAN_OPTIONS } onChange={this.onChange} />
        <CheckBoxInput model={model} name="in_utero" label={'Did reaction occur in utero?'} onChange={this.onChange} />
