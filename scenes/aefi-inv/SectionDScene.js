  ...
  render() {
    ...
    return (
      <KeyboardAwareScrollView style={ [AppStyles.scrollContainer, AppStyles.aefiBackground] } keyboardShouldPersistTaps={'handled'}>
        ...
        <TextInputField model={ model } name="vaccinated_vial" label="(h) Number vaccinated from the concerned vaccine vial/ampoule" keyboardType="numeric"/>
        <TextInputField model={ model } name="vaccinated_session" label="(i) Number vaccinated with the concerned vaccine in the same session" keyboardType="numeric"/>
        ...
        <TextInputField model={ model } name="vaccinated_cluster_number" label=" If yes, how many other cases have been detected in the cluster?" keyboardType="numeric"/>
